...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None, frequenices=[8,16,32]):
        """
    ...
    def generate_train_batch(self):
        ...
        data = np.zeros(self.data_shape, dtype=np.float32) # (2, 1, 28, 301, 301)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        pos_enc = np.zeros([self.data_shape[0], 3*2*len(self.frequenices), *self.data_shape[2:]], dtype=np.float32) 
        case_properties = []
        ...
        for j, i in enumerate(selected_keys):
            ...
            if isfile(self._data[i]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode) # (2, N, H, W) data/seg for each
            else:
                case_all_data = np.load(self._data[i]['data_file'])['data']
            
            _, N, H, W = case_all_data.shape

            # Create normalized coordinate arrays
            n = np.linspace(0, 1, N)
            h = np.linspace(0, 1, H)  # Normalize to [0,1)
            w = np.linspace(0, 1, W)
            w = np.where(w > 0.5, 1-w, w)
            
            # Create the meshgrid
            N_grid, H_grid, W_grid = np.meshgrid(n, h, w, indexing='ij')  # Shape: (N, H, W)

            # Stack into shape (H, W, C, 3)
            coords = np.stack((N_grid, H_grid, W_grid), axis=-1) # Shape: (N, H, W, 3)
            encoded_coords = []
            for freq in self.frequenices:
                # Apply sin and cos to each axis
                encoded_coords.append(np.sin(2 * np.pi * freq * coords))
                encoded_coords.append(np.cos(2 * np.pi * freq * coords))
            pos_enc_template = np.transpose(np.concatenate(encoded_coords, axis=-1), (3,0,1,2))[None]  # (1, 3*2*n_freq, N, H, W)
            ...
            case_all_data = np.copy(case_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                    ...
                                    valid_bbox_z_lb:valid_bbox_z_ub]) # (2, N, H, W)
            pos_enc_crop = np.copy(pos_enc_template[:, :, valid_bbox_x_lb:valid_bbox_x_ub,
                                                valid_bbox_y_lb:valid_bbox_y_ub,
            ...
            pos_enc[j] = np.pad(pos_enc_crop, ((0, 0), 
                                               (0, 0),
                                                (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
        ...
        # (2, 1, 48, 225, 225)
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'pos_enc':pos_enc}
