...
def convert_3d_to_2d_generator(data_dict, key):
    shp = data_dict[key].shape
    data_dict[key] = data_dict[key].reshape((shp[0], shp[1] * shp[2], shp[3], shp[4]))
    data_dict[f'{key}_orig_shape_data'] = shp
    return data_dict


def convert_2d_to_3d_generator(data_dict, key):
    shp = data_dict[f'{key}_orig_shape_data']
    current_shape = data_dict[key].shape
    data_dict[key] = data_dict[key].reshape((shp[0], shp[1], shp[2], current_shape[-2], current_shape[-1]))
    return data_dict
...
class Convert3DTo2DTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        return convert_3d_to_2d_generator(data_dict, self.key)
...
class Convert2DTo3DTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        return convert_2d_to_3d_generator(data_dict, self.key)
