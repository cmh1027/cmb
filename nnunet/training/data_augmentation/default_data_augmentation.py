...
def get_default_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                             ...
                             seeds_train=None, seeds_val=None, regions=None, use_pos_enc=False):
    assert params.get('mirror') is None, "old version of params, use new keyword do_mirror"
    tr_transforms = []

    if params.get("selected_data_channels") is not None: # False
        tr_transforms.append(DataChannelSelectionTransform(params.get("selected_data_channels")))

    if params.get("selected_seg_channels") is not None: # False
        tr_transforms.append(SegChannelSelectionTransform(params.get("selected_seg_channels")))

    # don't do color augmentations while in 2d mode with 3d data because the color channel is overloaded!!
    if params.get("dummy_2D") is not None and params.get("dummy_2D"): 
        tr_transforms.append(Convert3DTo2DTransform(key='data'))
        tr_transforms.append(Convert3DTo2DTransform(key='seg'))
        if use_pos_enc:
            tr_transforms.append(Convert3DTo2DTransform(key='pos_enc'))
        patch_size_spatial = patch_size[1:]
    else:
        patch_size_spatial = patch_size

    # Define positional arguments (args)
    args = (patch_size_spatial,)  # Assuming patch_size_spatial is a tuple or list

    # Define keyword arguments (kwargs)
    kwargs = {
        'patch_center_dist_from_border': None,
        'do_elastic_deform': params.get("do_elastic", False),
        'alpha': params.get("elastic_deform_alpha", 0),
        'sigma': params.get("elastic_deform_sigma", 0),
        'do_rotation': params.get("do_rotation", False),
        'angle_x': params.get("rotation_x", (0, 0)),
        'angle_y': params.get("rotation_y", (0, 0)),
        'angle_z': params.get("rotation_z", (0, 0)),
        'do_scale': params.get("do_scaling", False),
        'scale': params.get("scale_range", (1, 1)),
        'border_mode_data': params.get("border_mode_data", 'constant'),
        'border_cval_data': params.get("border_cval_data", 0),
        'order_data': params.get("order_data", 3),
        'border_mode_seg': params.get("border_mode_seg", "constant"),
        'border_cval_seg': params.get("border_cval_seg", 0),
        'order_seg': params.get("order_seg", 1),
        'random_crop': params.get("random_crop", False),
        'p_el_per_sample': params.get("p_eldef", 0),
        'p_scale_per_sample': params.get("p_scale", 0),
        'p_rot_per_sample': params.get("p_rot", 0),
        'independent_scale_for_each_axis': params.get("independent_scale_factor_for_each_axis", False)
    }

    tr_transforms.append(SpatialTransform(*args, **kwargs))
    if use_pos_enc:
        tr_transforms.append(SpatialTransform(*args, data_key='pos_enc', label_key="", **kwargs))

    if params.get("dummy_2D") is not None and params.get("dummy_2D"): 
        tr_transforms.append(Convert2DTo3DTransform(key='data'))
        tr_transforms.append(Convert2DTo3DTransform(key='seg'))
        if use_pos_enc:
            tr_transforms.append(Convert2DTo3DTransform(key='pos_enc'))

    if params.get("do_gamma"): # True
        tr_transforms.append(
            GammaTransform(params.get("gamma_range"), False, True, retain_stats=params.get("gamma_retain_stats"),
                           p_per_sample=params["p_gamma"]))

    if params.get("do_mirror"): # True
        tr_transforms.append(MirrorTransform(params.get("mirror_axes")))
        if use_pos_enc:
             tr_transforms.append(MirrorTransform(params.get("mirror_axes"), data_key='pos_enc', label_key=""))

    if params.get("mask_was_used_for_normalization") is not None: # False
    ...
    if use_pos_enc:
        tr_transforms.append(NumpyToTensor(['pos_enc'], 'float'))
    ...
    if use_pos_enc:
        val_transforms.append(NumpyToTensor(['pos_enc'], 'float'))
