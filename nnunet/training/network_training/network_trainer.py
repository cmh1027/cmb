...
class NetworkTrainer(object):
    ...
    def load_best_checkpoint(self, train=True):
        ...
        if isfile(join(self.output_folder, "model_best.model")):
            self.load_checkpoint(join(self.output_folder, self.checkpoint_base, "model_best.model"), train=train)
        else:
    ...
    def load_latest_checkpoint(self, train=True):
        if isfile(join(self.output_folder, "model_final_checkpoint.model")):
            return self.load_checkpoint(join(self.output_folder, self.checkpoint_base, "model_final_checkpoint.model"), train=train)
        if isfile(join(self.output_folder, "model_latest.model")):
            return self.load_checkpoint(join(self.output_folder, self.checkpoint_base, "model_latest.model"), train=train)
        if isfile(join(self.output_folder, "model_best.model")):
            return self.load_best_checkpoint(train)
        raise RuntimeError("No checkpoint found")

    def load_final_checkpoint(self, train=False):
        filename = join(self.output_folder, self.checkpoint_base, "model_final_checkpoint.model")
    ...
    def load_checkpoint(self, fname, train=True, lr_mult=1.0):
        # self.print_to_log_file("loading checkpoint", fname, "train=", train)
        if not self.was_initialized:
            self.initialize(train)
        # saved_model = torch.load(fname, map_location=torch.device('cuda', torch.cuda.current_device()))
        saved_model = torch.load(fname, map_location=torch.device('cpu'))
        print(f"Load checkpoint {fname} (EPOCH {saved_model['epoch']})")
        self.load_checkpoint_ram(saved_model, train, lr_mult=lr_mult)
    ...
    def load_checkpoint_ram(self, checkpoint, train=True, lr_mult=1.0):
        """
    ...
    def run_training(self):
        ...
        if self.save_final_checkpoint: self.save_checkpoint(join(self.output_folder, self.checkpoint_base, "model_final_checkpoint.model"))
    ...
    def maybe_save_checkpoint(self):
        ...
        if self.save_intermediate_checkpoints and (self.epoch % self.save_every == (self.save_every - 1)):
            ...
            if not self.save_latest_only:
                self.save_checkpoint(join(self.output_folder, self.checkpoint_base, "model_ep_%03.0d.model" % (self.epoch + 1)))
            self.save_checkpoint(join(self.output_folder, self.checkpoint_base, "model_latest.model"))
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                ...
                if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, self.checkpoint_base, "model_best.model"))
            ...
            if self.epoch - self.best_epoch_based_on_MA_tr_loss > self.patience:
                if self.optimizer.param_groups[0]['lr'] > self.lr_threshold:
                    ...
                    print("No improvements. Early stopping.")
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        pos_enc = data_dict['pos_enc'] # (2, n_freq, 48, 225, 225)
        data = maybe_to_torch(data) # (2, 1, 48, 192, 192)
        target = maybe_to_torch(target)
        pos_enc = maybe_to_torch(pos_enc)

        if torch.cuda.is_available():
            data = to_cuda(data)
            target = to_cuda(target)
            pos_enc = to_cuda(pos_enc)

        self.optimizer.zero_grad()

        if self.fp16:
            with autocast():
                output = self.network(data, pos_enc)
                del data
