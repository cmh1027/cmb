...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              ...
                              base_module='nnunet.training.network_training', plan_task=None):
    ...
    plan_task = task if plan_task is None else plan_task

    if network == '2d':
        plans_file = join(preprocessing_output_dir, plan_task, plans_identifier + "_plans_2D.pkl")
    else:
        plans_file = join(preprocessing_output_dir, plan_task, plans_identifier + "_plans_3D.pkl")
    ...
    print("\nI am using plans from this folder: ", plans_file)
