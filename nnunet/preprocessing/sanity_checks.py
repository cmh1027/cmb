...
def verify_dataset_integrity(folder, skip_data=False, skip_label=False):
    """
    ...
    if not skip_data:
        ...
        for c in tqdm(expected_train_identifiers, desc="checking case"):
    ...
    #         expected_image_files = [join(folder, "imagesTr", c + "_%04.0d.nii.gz" % i) for i in range(num_modalities)]
            ...
            for i, img in enumerate(images_itk):
                ...
                if not same_geometry:
                    raise AssertionError("The geometry of the image %s does not match the geometry of the label file. The pixel arrays "
                        "will not be aligned and nnU-Net cannot use this data. Please make sure your image modalities "
                        "are coregistered and have the same geometry as the label" % expected_image_files[i])
